...
Reads your current identity files and MEMORY.md, then reads recent session logs from `daily/`, and proposes concrete updates to keep your Master Prompt current with how you're actually working.
...
2. **Read recent sessions** — the 5 most recent files in `daily/` (or all of them if there are fewer than 5). If `daily/` doesn't exist yet or is empty, say so and stop — there's nothing to review.

3. **Analyse**. Look for:
   - **New preferences** revealed by how the user corrected, pushed back, or praised responses — material for SOUL.md
   - **New projects / goals / areas** the user has started working on — material for GOALS.md
...
5. **Wait for approval**. Do NOT write any changes yet. Ask:
   > *"Apply these changes? Say 'yes', 'yes but skip X', or specify which to apply."*

6. **On approval**, write the approved changes. Confirm what was written.
...
If it's been more than 2 months since the last run and `daily/` has 20+ files, summarise what you see more aggressively — the user's identity may have drifted meaningfully.
