 ...
 * convention used by LoadMasterPrompt. Returns null if no vault found.
 */
export function findVaultDir(cwd: string): string | null {
  try {
    const entries = readdirSync(cwd);
    const vault = entries.find((d) => d === 'vault' || d.endsWith('-vault'));
    return vault ? join(cwd, vault) : null;
  } catch {
    return null;
  }
}
