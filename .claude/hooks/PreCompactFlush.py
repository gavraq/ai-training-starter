...
Python equivalent of PreCompactFlush.ts. Same behaviour, same output.
...
.claude/settings.json and change the PreCompact hook command to
`python3 $CLAUDE_PROJECT_DIR/.claude/hooks/PreCompactFlush.py`.
"""
...
def main() -> None:
    ...
    heading = (
        f"### Session {short_id} "
        f"({hhmm(summary.start_time)}–{hhmm(summary.end_time)}) — pre-compact"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"[PreCompactFlush] {err}", file=sys.stderr)
